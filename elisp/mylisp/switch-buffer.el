...
;; Last-Updated: 2015/10/02 13:31:22 (+0900)
;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;
...
(defun t1-switch-04_bindings_start ()
  ...
  (interactive)
  (t1-switch--findfile "04_bindings_start.el" my-start-dir))

;;;###autoload
(defun switch-test_start ()
  "Switch test_start.el or find file."
  (interactive)
  (t1-switch--findfile "test_start.el" my-start-dir))

;;;###autoload
(defun switch-hotstring () ;[2015/07/31]
  "Switch to 30_hotstring_plugin_start.el"
  (interactive)
  (t1-switch--findfile "30_hotstring_plugin_start.el" my-start-dir))
...
(defun find-file-conf ()
  ...
  (let ((mj (if (eq major-mode 'emacs-lisp-mode)
        ...
        finded-file)
    (setq finded-file (match-dir-file mj my-start-dir "^[[:digit:]]\\{2\\}\\|\\.elc$"))
    (if finded-file
        (find-file finded-file)
      (find-file (concat my-start-dir mj "_start.el")))))
