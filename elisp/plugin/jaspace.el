...
;;; $Id: jaspace.el 881 2013-11-23 10:57:42Z t1 $
...
(defface jaspace-highlight-tab-face
  '((t (:foreground "dark sea green" :strike-through t)))
...
(defconst jaspace-font-lock-keywords
  '(("　" 0 'jaspace-highlight-jaspace-face prepend)
	("\n" 0 'jaspace-highlight-eol-face t)
	("\t" 0 'jaspace-highlight-tab-face t)
	("\t$" 0 (if (and (boundp 'show-trailing-whitespace)
					  show-trailing-whitespace)
				 'trailing-whitespace
			   'jaspace-highlight-tab-face) t)))
...
othrewise off."
  (interactive "P")
  (if (null arg) (setq arg jaspace-mode))
...
(defun jaspace-mode-on ()
  ...
  (unless jaspace-mode
	...
	(setq jaspace-ignore-command t)
	(unless (null jaspace-mode-hook) (run-hooks 'jaspace-mode-hook))
	(setq jaspace-ignore-command nil)
...
(defun jaspace-mode-enter ()
  (let ((jaspace-ignore-command t))
	...
	(font-lock-add-keywords
	 nil (list (assoc "　" jaspace-font-lock-keywords)))
	...
	;; and make tab characters visible if necessary.
  ...
  (let ((jaspace-ignore-command t))
	...
	(if (fboundp 'font-lock-remove-keywords)
	  ...
	  (progn
		...
		(dolist (keyword jaspace-font-lock-keywords)
...
(defun jaspace-update ()
  (when (< jaspace-base-mode 0)
	(if (memq major-mode jaspace-modes)
		(setq jaspace-base-mode 1)
	  (setq jaspace-base-mode 0)))
  (cond ((and buffer-read-only jaspace-disable-on-read-only-buffers)
		 (jaspace-mode-off))
		(jaspace-follow-font-lock
		 (when (and (< 0 jaspace-base-mode)
					(not (equal jaspace-mode font-lock-mode)))
...
(defun jaspace-post-command-hook ()
  (unless jaspace-ignore-command (jaspace-update))
)

;; add me to the list of minor modes.
(when (boundp 'minor-mode-alist)
  (add-to-list 'minor-mode-alist '(jaspace-mode jaspace-mode-string)))

(when (boundp 'mode-line-mode-menu)
  ;; for Emacs 21.
  (define-key mode-line-mode-menu [jaspace-mode]
	'(menu-item "Japanese Whitespaces" jaspace-mode
				:button (:toggle . jaspace-mode))))

;; when any command has been invoked, restore the previous jaspace status
;; accodring to the buffer's read-only flag.
