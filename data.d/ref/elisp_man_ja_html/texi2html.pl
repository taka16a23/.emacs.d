...
{
 # titles  of pages
 'ToC_Title' => 'Emacs Lisp リファレンスマニュアル',
 'Overview_Title' => '目次',
 'Index_Title' => '索引',
 'About_Title' => 'この文書について',
 'Footnotes_Title' => '脚注',
 'See' => 'See',
 'see' => 'see',
 'section' => '節',
...
sub T2H_DEFAULT_print_Top
{
  ...
  if ($T2H_THIS_SECTION)
  {
    ...
    print $fh "<H1>$T2H_NAME{Top}</H1><FORM METHOD=\"GET\" ACTION=\"http://www.bookshelf.jp/cgi-bin/wwwsrch.cgi\"><div class=\"info\">検索<INPUT TYPE=text NAME=WORD SIZE=30><INPUT TYPE=hidden NAME=dir VALUE=$texi_name_sarchcgi><INPUT TYPE=hidden NAME=index VALUE=dirsearch CHECKED><INPUT TYPE=hidden NAME=target VALUE=all CHECKED><INPUT TYPE=hidden NAME=ANDOR VALUE=and CHECKED><INPUT TYPE=radio NAME=lisp VALUE=off CHECKED>全文<INPUT TYPE=radio NAME=lisp VALUE=on>Elisp<INPUT TYPE=submit VALUE=\"検索\"></div></FORM>"
...
sub T2H_DEFAULT_print_navigation
{
  ...
  print $fh "<TD> [<a href=\"http://www.bookshelf.jp/cgi-bin/goto.cgi?file=meadow&node=search\">検索</a>] [<a href=\"#top\">上端</a> / <a href=\"#bottom\">下端</a>]</TD>";
  print $fh "</TR>" unless $vertical;
  print $fh "</TABLE>\n";
}
		...
		else
		{
		  ...
		  else
		  {
		    push(@lines, &html_debug("<A NAME=\"$docid\"> </A>\n", 
                 ...
                 if (-e $filename) {
		     @foo = stat($filename);	# statでファイル情報取得
		     ($sec, $min, $hour, $mday, $mon, $year, $wday, $yday, $isdst)
			 = localtime($foo[9]);# 更新時刻
		     $year += 1900;
		     $mon += 1;
                     $size = (-s $filename);
                     $size = $size / 1000;
...
メールアドレス：<INPUT TYPE=\"text\" NAME=\"EMAIL\" SIZE=\"25\" MAXLENGTH=\"60\" VALUE=\"secret\@mail.adr\"><br>
<input type=\"hidden\" name=\"HPAGE\" value=\"\">
これを追加して欲しい：<br><TEXTAREA NAME=\"MESSAGE\" ROWS=\"10\" COLS=\"50\">
</TEXTAREA><br><br>
<INPUT TYPE=\"submit\" VALUE=\"送信\">
<INPUT TYPE=\"reset\" VALUE=\"クリア\">
</FORM> 
