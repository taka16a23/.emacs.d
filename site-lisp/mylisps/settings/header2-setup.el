;;; header2-setup.el ---   setting for header2
;;
...
(use-package header2
  ...
  (t1-edit-bind-keys '(("R" . make-revision)
                       ("c" . make-revision)
                       ("C" . make-revision)))
  :config
  (message "Loading \"header2\"")
...
with GNU Emacs.  If not, see <http://www.gnu.org/licenses/>.
 ")
  (register-file-header-action
   (concat "^" header-prefix-string ".+Last-Updated[ \t]*:")
...

(provide 'header2-setup)
;; For Emacs
;; Local Variables:
;; coding: utf-8
;; End:
;;; header2-setup.el ends here
